...
export default function UploadPhoto() {
  ...
  const handleProceed = () => {
    const eventSelect = document.querySelector("select").value; // Get selected event value
  
    if (!file && !selectedDate && eventSelect === "Select Event") {
      alert("Please fill at least one field: Event, Date, or Upload a Photo.");
      return;
    }
        ...
        <div className="p-4  bg-white">
          <div className="relative w-full mb-4">
            <select className="w-full p-3 border border-gray-300 rounded-full appearance-none text-[#170645] bg-white focus:outline-none">
              ...
              <option >Azadi Ka Amrit Mahotsav</option>
              <option >Rajim Kumbh Mela</option>
              <option >Chhattisgarh Yojna</option>
              <option >Mahatari Vandan Yojna</option>
              <option >Harihar Chhattisgarh</option>
              <option >Rajutsav 2025</option>
            </select>
          ...
          <div className="columns-1 sm:columns-2 md:columns-3 lg:columns-4 gap-4 space-y-4 mt-4">
            {currentImages.map((image, index) => (
              <div key={index} className="break-inside-avoid bg-white p-4 rounded-lg">
                <img src={image.src} alt={image.title} className="w-full rounded-[30px]" />

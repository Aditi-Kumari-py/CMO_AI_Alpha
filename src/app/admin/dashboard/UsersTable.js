...
export default function UsersTable() {
  ...
  const toggleStatus = async (id, currentStatus) => {
    try {
      const response = await fetch(`https://backend-znxf.onrender.com/update-user/${id}`, {
        method: "PUT",
  ...
  const fetchUsers = async () => {
    try {
      const response = await fetch("https://backend-znxf.onrender.com/users");
  ...
  if (password && password.length > 0) {
    ...
    try {
      const response = await fetch(`https://backend-znxf.onrender.com/update-user/${editedUser._id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("https://backend-znxf.onrender.com/add-staff", {
        method: "POST",
        headers: { "Content-Type": "application/json" },

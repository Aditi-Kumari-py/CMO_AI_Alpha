...
const AllPhotos = ({ currentTab } ) => {
  ...
  const fetchAlbums = async () => {
    try {
      const response = await fetch("https://backend-znxf.onrender.com/albums");
      if (!response.ok) {
  ...
  try {
    const response = await fetch(`https://backend-znxf.onrender.com/photos/${album._id}`);
  ...
  const handleCreateAlbum = async (newAlbum) => {
    try {
      const response = await fetch("https://backend-znxf.onrender.com/create-album", {
        method: "POST",
  ...
  const handleSelectAll = () => {
    if (selectedAlbum) {
      ...
      const handleDownload = async () => {
        ...
        const zip = new JSZip();
        for (let albumId of selectedItems) {
          const album = albums.find((album) => album._id === albumId);
          if (!album) continue;

          zip.file(`${album.name}_cover.jpg`, album.cover, { base64: true });

          try {
            const response = await fetch(`https://backend-znxf.onrender.com/photos/${albumId}`);
            if (!response.ok) throw new Error("Failed to fetch photos");
            const data = await response.json();

            data.images.forEach((img, index) => {
              zip.file(`${album.name}_photo_${index + 1}.jpg`, img, { base64: true });
            });
          } catch (error) {
          ...
          if (selectedAlbum) {
            ...
            for (const photo of selectedItems) {
              ...
              const response = await fetch(
                `https://backend-znxf.onrender.com/photo/${selectedAlbum._id}/${photo.photo_id}`,
                { method: "DELETE" }
              );
            ...
            // Deleting albums
            const response = await fetch("https://backend-znxf.onrender.com/delete-albums", {
              method: "DELETE",
              headers: { "Content-Type": "application/json" },

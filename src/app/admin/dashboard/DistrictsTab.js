...
export default function DistrictsTab() {
  ...
  useEffect(() => {
    fetch("https://backend-znxf.onrender.com/districts")
      .then((res) => res.json())
  ...
  const handleSaveEdit = (index) => {
    if (editedName.trim() !== "" && index >= 0 && index < districts.length) {
      fetch(`https://backend-znxf.onrender.com/districts/${districts[index].name}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = (name) => {
    fetch(`https://backend-znxf.onrender.com/districts/${name}`, { method: "DELETE" })
